...
public class Phase2GameManager : MonoBehaviour
{
    ...
    private void SpawnLetherGun()
    {
        ...
        GameObject LetherGun = Instantiate(letherGun, new Vector3(-7.5f, RandomY, 0f),
            Quaternion.identity);
    ...
    private void SpawnEnemy()
    {
        if (enableSpawnEnemy && !Phase2PlayerController.isDead && !isRedTankDown)
        {
            ...
            GameObject spawnEnemy = (GameObject)Instantiate(enemy, new Vector3(10f,
                randomY, 0f), Quaternion.identity); 
        }
    }
    ...
    private void SpawnTank()
    {
        if (maxTank <= 2)
        {
            if (enableSpawnTank && !Phase2PlayerController.isDead && !isRedTankDown)
            {
                ...
                GameObject spawnTank = (GameObject)Instantiate(tank,
                    new Vector3(gameObject.transform.position.x + spawnXAxis,
                    randomY, 0f), rotation);
                maxTank += 1;
            }
        }
    ...
    private void SpawnRedTank()
    {
        if (enableSpawnRedTank && !Phase2PlayerController.isDead)
        {
            ...
            Quaternion rotation = Quaternion.Euler(0, 0, randomZ);
            GameObject spawnTank = (GameObject)Instantiate(redTank,
                new Vector3(gameObject.transform.position.x + spawnXAxis,
                randomY, 0f), rotation);
        }
    }
